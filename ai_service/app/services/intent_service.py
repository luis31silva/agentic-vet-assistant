...
- **CREATE_OWNER_AND_PATIENT**: extrair `owner: {ownerName, nif, phoneNumber, phoneNumber2, email}` e `patient: {patientName, species, breed, weight, birthDate, microchip}`
- **CREATE_OWNER**: extrair `owner: {ownerName, nif, phoneNumber, phoneNumber2, email}`
- **CREATE_PATIENT**: extrair `patient: {patientName, species, breed, weight, birthDate, microchip, ownerId}` e `ownerName` se mencionado
- **ADD_VACCINES**: extrair `patientName` ou `patientId`, e `vaccines: [{name, date}]`
- **SEARCH_PATIENT**: extrair `patientName`, `species`, `breed`
- **SEARCH_OWNER**: extrair `ownerName`, `nif`
- **GET_PATIENT_HISTORY**: extrair `patientName` ou `patientId`
- **GET_APPOINTMENTS**: extrair `patientName` ou `patientId`
- **GET_OWNER_PATIENTS**: extrair `ownerName` ou `ownerId`
- **CLINICAL_ADVICE**: extrair `patientName` ou `patientId`, `symptoms`
...
Exemplo:
- Entidades anteriores: { "patient": { "patientName": "Rex" } }
- Mensagem: "É um labrador"
- Resposta: entities = { "patient": { "patientName": "Rex", "breed": "Labrador" } }
